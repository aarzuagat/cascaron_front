<template>
  <div>
    ...
    <q-btn class="full-width" dense no-caps @click="selling = true" flat text-color="white"
           color="red"
           align="left">
...
import {destructurateObject, objectToJSON} from "src/utils/utils";
  ...
  methods: {
    ...
    secondPart() {
      if (this.productReal.tag !== 'Todas las unidades') {
        ...
        const lista = [...Array(parseInt(this.stock)-1).keys()]
        this.sell = []
        const item = {
          id: 0,
    ...
    async submit() {
      ...
      console.log(obj)
      await updateStock(objectToJSON(obj))
      this.part = 0;
      this.selling = false
      this.tag = ''
      this.products_filtered = this.products_all = []
